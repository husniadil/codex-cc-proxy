...
base=https://github.com/husniadil/codex-cc-proxy/releases/latest/download

curl -fLO "$base/codex-cc-proxy-$target.tar.gz"
curl -fLO "$base/SHA256SUMS"
shasum -a 256 --ignore-missing -c SHA256SUMS   # sha256sum on Linux

tar -xzf "codex-cc-proxy-$target.tar.gz"
install "codex-cc-proxy-$target/codex-cc-proxy" /usr/local/bin/
```
...
```sh
cargo install --git https://github.com/husniadil/codex-cc-proxy --locked codex-cc-proxy
```
