...
| How is a grant for the second provider obtained? | **Designed, deferred — no consumer.** The operator has one subscription on this provider, so the multi-account machinery has nothing to serve; building it now would be a path with no traffic. Three approaches were weighed. A PKCE flow of the proxy's own is rejected outright: it would run under the harness client's identity, which is a line not to cross. Capturing a copy of the harness's grant and refreshing it here works but makes the proxy a second refresher of one refresh-token family — the one-grant-two-stores hazard that ruled out import for the first provider — and the refresh call itself must present the harness's public OAuth client id. `setup-token` is declined by the operator. The chosen shape, when a consumer appears: the proxy **holds no copy and never refreshes**. A stored account for this provider is a *reference* to the harness's own credential storage (macOS login keychain item `Claude Code-credentials` via the `security` CLI, or `~/.claude/.credentials.json`), and the authorizer reads the live bearer from it per turn, cached until near its `expiresAt`. The harness stays the sole refresher, using its own client — the proxy borrows nothing. Keeping the token fresh is an operator-side scheduled `claude` invocation (the harness refreshes its own token on use, evidenced by no re-login after an overnight gap); whether a bare start-and-exit refreshes, or a one-turn `claude -p` is needed, is a 30-second probe left for when this is built. This serves one live account only — whichever the harness holds; a pool of subscriptions on this provider would still need credential-swapping and is a separate question. |
