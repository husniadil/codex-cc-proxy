...
| Do the `session-id` and `thread-id` headers matter? | **Open.** The reference client sends both on every upstream request and uses the session id as its prompt cache key; this proxy sends neither, and instead sends `prompt_cache_key` in the body, which is what actually drives caching (§2.1). Turns complete without them, so nothing is visibly broken — but whether they affect routing, rate-limit accounting, or cache behaviour is unmeasured. Read from the upstream repo, not observed on the wire. |
