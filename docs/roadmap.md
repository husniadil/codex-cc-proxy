...
| Does it accept an `input_file` part, the one shape with no upstream precedent? | **Still open, and harder to close than expected.** Claude Code rasterises PDFs into `image` blocks, so reading a PDF exercises the image path — verified live, code `PDF-KESTREL-8823` returned from a PDF nothing else contained. Nothing this client sends reaches `input_file`; closing it needs a client that emits `document` blocks. |
