...
| Does any client read `anthropic-ratelimit-unified-*` from a proxy? | **Open, and currently no.** A stub endpoint setting those headers left `rate_limits` absent from the status-line payload, so this client version does not surface a quota that did not come from its own account. The headers are emitted anyway — they are the accurate form of a figure the response carries — and `usage` is the path that works. Worth rechecking when the client's auth handling changes. |
| Is `ultra` gated by plan as well as by model? | **Open, and not settleable here.** The catalog advertises `ultra` on `gpt-5.6-terra`, and this account's requests for it are refused with `Invalid value: 'ultra'` — the schema-level refusal, not the model-specific one that `minimal` gets. It is reported to need a paid plan and a model that has it. The catalog is therefore a menu of what a client may offer, not a statement of what the wire accepts, and nothing here can tell the two apart. The refusal is surfaced verbatim rather than guessed around. |
