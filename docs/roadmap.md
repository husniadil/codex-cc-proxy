...
| Can a key list models at the catalog endpoint? | **Open.** The catalog fetch answered `401 Unauthorized` for `/v1/models` while the same key authenticated a turn at `/v1/responses` — so the key is valid and something else refuses the list. Two candidates, neither settled: a key scoped without model-read permission, or something this proxy sends on that request. The catalog falls back and says so, which is the documented behaviour, so nothing is silently wrong. |
